...
The situation with clang is a bit (!) more complicated. Not only does clang need to be able to find its own include files and libraries, but it also needs to be able to find the files for the compiler that clang is being built with. In order to successfully build clang with a non-standard compiler, we are going to need to specify the following parameters to the clang build:
